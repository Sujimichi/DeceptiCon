  ...
  def assert_mapping
    ...
    actions.each do |action| 
      ...
      mapping.each do |format, expected_outcome|
        ...
        msg_txt = (expected_outcome.eql?(true) ? "" : "NOT ") +  "respond" if expected_outcome.eql?(true) || expected_outcome.eql?(false)
        msg_txt = "redirect requests" if expected_outcome.eql?(:redirect)
        msg_txt ||= "return the status code #{expected_outcome} for requests"
        
        #puts "should #{msg_txt} to #{action}:#{format}"
        it "should #{msg_txt} to #{action}:#{format}" do #Define an rspec test step
          inner_step_logic(object_class, action, format, expected_outcome, mapping.clone)
        end
      end
    end
  end

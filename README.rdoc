...
The +assert_mapping+ method assumes that it will be able to call methods named <tt>valid_<class_name.underscore></tt>, ie +valid_note+.  
It is the expectation that a valid_object method will return a Factory generated object which returns true for .valid?
If such a method can't be found it will attempt to create an object and populate it attributes to make it valid but this is only very basic.

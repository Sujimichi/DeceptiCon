=DeceptiCon
A way to simplify your Rails controller tests and make them DRY.
...
* You want to be able to simply test the basic <tt>should be_success</tt> or <tt>should_not be_success</tt> assertions for each action and each format (at least html and js).
  ...
  * it takes at least 10 lines of controller_spec to test just one action with two formats, ie;
    ...
    Just testing the basic responses is going to take at least 70 lines!

* Controller testing is so boring and repetitive it makes you cry.  You just want to focus on the interesting behaviors of your controllers and leave this repetitive stuff to a robot.
...
Here is a controller_spec which tests the html and js (ajax) responses to each of the seven actions and asserts if the response should be successful or not.
  ...
  describe NotesController do
    ...
    @object = Note
    @test_formats = [:html, :ajax] #could also include :xml and :json
    @action_mapping = {
...
Two tests have been generated for each of the seven controller actions to test thier response to both html and js requests.
...
* For :create the params include attributes for the appropriate object.
* For :update the params include attributes for the object and an :id.
* All other actions (:show, :edit, :destroy) just get an :id.

====Important Prerequisite
For all the actions, aside from :index and :new, the test will need to construct an object in the test DB for the controller.
If you have Factories set up for your classes it will use <tt>Factory.build(class_name)</tt>.
It will however first look for a method called <tt>valid_<class_name></tt> ie +valid_note+ or +valid_user_comment+.

It is the expectation that the object returned by either a Factory or a valid_object method will return true for .valid?
It it can't find a Factory or valid_object method it will attempt to create an object and populate its attributes (with string or ints) to try to make it valid but this only works in very simple situations.


The generation of the tests is done by the +assert_mapping+ method in the DeceptiCon module which needs to be required and included in the spec_helper.
...
====Be Lazy
You only need to define the mapping for actions which have the expectation of a susscessful response.  The others will be assumed to not have a succsessful response.

  @action_mapping = { :index => {:html => true} }

Without other options, just using the above action_maping would result in all seven actions being tested for thier response to each of the four formats (html, js, xml, json) with only the html response to index expected to be succsessful.

By default all four formats (html, js, xml, json) are tested for all seven actions.  You can adjust this for all controllers or just for the current.  See more in Options later.
...
==Options
By Default all seven actions will be tested for all four format, you might not want this.
...
Which actions are tested can also be set on a per-controller basis.  By default all seven actions are tested by you can include an array (@skip_actions) to have some actions ignored.
  @skip_actions = [:edit, :udate, :destroy] #these three will not be tested.

...
For each of the seven controller actions in <tt>@action_mapping<\tt> a test has been created for each of its two formats.  
...
<tt>@action_mapping</tt> is a hash with a key for each action which entails another hash which defines the formats which the action will <tt>respond_to<\tt> and the expection of the request.  

  {:action => {:format => expectation of request}}

The expectation of e request is either +true+ or +false+ to denote if the <tt>response.should be_succsess<\tt> or if the <tt>response.should_not be_succsess<\tt>. 

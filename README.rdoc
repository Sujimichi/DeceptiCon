...
====Be Lazy
You only need to define the mapping for actions which have the expectation of a susscessful response.  

  @action_mapping = { :index => {:html => true} }

The assumes that the other actions are expected to not be succsessful for either format and only html will be succsessful for index.

...
Each of the 14 generated tests attempts to make the appropriate request (get, put, post or delete) and asserts if it should be_success or not.
...
====Be Lazy
You only need to define the mapping for actions which have the expectation of a successful response.  The others will be assumed to not have a successful response.

  @action_mapping = { :index => {:html => true} }

Without other options, just using the above action_mapping would result in all seven actions being tested for their response to each of the four formats (html, js, xml, json) with only the html response to index expected to be successful.

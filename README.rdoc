...
==DRY your eyes
Here is a controller_spec which tests each of the seven actions, each with either format :html or :js (:ajax) and asserts if that action should be successful or not.
...
====Be Lazy
You only need to define the mapping for actions which have the expectation of a successful response.

  @action_mapping = { :index => {:html => true} }

The assumes that the other actions are expected to not be successful for either format and only html will be successful for index.
